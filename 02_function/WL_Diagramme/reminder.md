...
| `cloud_diagram.R` | `Cloud_diagram_function()` — Klima-Wolken-Diagramm aus dem MAT/MAP-RDS (6.6): BL via NUTS1-Lookup (`nuts_id.RDS`, je Punkt – auch ohne Boden), MASTER_ID via Boden-CSV, einmal angereichert + Cache-RDS. Referenz-Klimaraum (blaue 95%-Box + Mittel) und optional Vergleichslauf (darkred Box + Beschriftung + erwartete MASTER_ID-Lage + Pfeil + Δ-Text). Eingebunden in 6.15. |
