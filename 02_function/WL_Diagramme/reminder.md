...
| `cloud_diagram.R` | `Cloud_diagram_function()` — Klima-Wolken-Diagramm (x=MAP, y=MAT) aus dem MAT/MAP-RDS (6.6): BL via NUTS1-Lookup (`nuts_id.RDS`, je Punkt – auch ohne Boden), MASTER_ID via Boden-CSV, einmal angereichert + Cache-RDS. Wolke (DE dunkelgrau, BL hellgrau) + hervorgehobene **MASTER_ID-Station** (farbiger Punkt, Label aus `New_label`); `Klimalauf_compare` (Vektor) zeichnet die erwartete Stationslage je Vergleichslauf + Pfeil (`show_shift`) = **Mittelpunktverschiebung**. Eine Farb-Legende (DE/BL/Station bzw. Lauf, Lauf-Namen ohne Modell via `run_kurz`), Subtitle = Δ der Station (Vergleich−Referenz), Text-Block unten links (DE/BL/Station-Zahlen). `point_size`/`cloud_size` als Parameter. Eingebunden in 6.15. |
| `run_klimadiagramme.R` | **Orchestrator** (6.1–6.15): laden → WL/Trend → Region/Lauf-CSVs (6.1–6.9); je-MASTER_ID-Einzeldiagramme (6.10); Lauf-Vergleich Ref vs. Zukunft (6.11); gestaffelter WL + De-Martonne (6.12); T/P-Debug (6.13); Regions-Mittel-WL je Lauf (6.14); Klima-Wolken-Diagramm (6.15) |
